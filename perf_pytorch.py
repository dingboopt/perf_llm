...
times = int(40*312*(10**12)/(m*n*k*2))

with torch.cuda.device(1):
    x = torch.rand((m , k), device=cuda, dtype=torch.bfloat16)
...
now_time = datetime.datetime.now(timezone(zone))
print (now_time.strftime("%Y-%m-%d %H:%M:%S"))
d = {'M': [m], 'K':[k], 'N':[n], 'Flops':[tflops], 'SOL':[tflops/active*1000000/self_cuda_time/312], 'TimeStamp':[now_time.strftime("%Y-%m-%d %H:%M:%S")]}
